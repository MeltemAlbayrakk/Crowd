...
const addPersonalDetail = async (req, res) => {
  ...
  try {
    ...
    const { _id, firstName, lastName, birthday, gender, languages, skills, profileDescription, address } = req.body;
    if (_id) {
      const personalDetails = await UserModel.findByIdAndUpdate(_id, {
...
const beFreelancer = async (req, res) => {
  ...
  try {
    ...
    const { _id, profession, description, speciality } = req.body;

    if (_id) {
      const becomeFreelancer = await UserModel.findByIdAndUpdate(_id, {
...
export {
  registerCompanyUser, registerPersonelUser, login, logout, addPersonalDetail, getProfile, checkUser, addProfilePicture, addCompanyDetail,
